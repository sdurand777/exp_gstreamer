...
def display_side_by_side(window, video_frame, klv_filename):
    ...
    cv2.waitKey(1)
    img_name = klv_filename.split("/")[-1]
    if "image_0" in klv_filename.split("/"):
        path_img_save = "/home/smith/exp_gstreamer/save/left"+img_name
    else:
        path_img_save = "/home/smith/exp_gstreamer/save/right"+img_name
    cv2.imwrite(path_img_save, combined)
    print("-------------- img saved at ", path_img_save)
    ...
    def on_new_video_sample(sink):
        ...
        if candidates:
            ...
            if diff <= TOLERANCE:
                ...
                # path_img_save = "/home/smith/exp_gstreamer/save/"+side+img_name
...
def main():
    ...
    video_left = Gst.parse_launch(
        f"srtsrc latency=1000 uri={SRC_URI_LEFT} ! queue ! application/x-rtp,media=video,encoding-name=JPEG,payload=26 "
        "! rtpjpegdepay ! jpegparse "
        "! queue ! appsink name=video_left_sink caps=\"image/jpeg\" emit-signals=true sync=true"
    )
    ...
    video_right = Gst.parse_launch(
        f"srtsrc latency=1000 uri={SRC_URI_RIGHT} ! queue ! application/x-rtp,media=video,encoding-name=JPEG,payload=26 "
        "! rtpjpegdepay ! jpegparse "
        "! queue ! appsink name=video_right_sink caps=\"image/jpeg\" emit-signals=true sync=true"
    )
