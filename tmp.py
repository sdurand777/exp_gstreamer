...
# --- Répertoire de sauvegarde et compteurs ---
...
# --- Callback vidéo générique avec sauvegarde concaténée ---
def make_video_callback(side, window):
    def on_new_video_sample(sink):
        ...
        # Stocke la frame
        last_frames[side] = frame
        if side == 'left':
            frame_counts['left'] += 1
            # Sauvegarde concaténée toutes les 10 images gauche
            if frame_counts['left'] % 10 == 0 and last_frames['right'] is not None:
                combo = np.hstack((last_frames['left'], last_frames['right']))
                fname = os.path.join(SAVE_DIR, f"concat_{frame_counts['left']:06d}.jpg")
                cv2.imwrite(fname, combo)
                print(f"[SAVE CONCAT] {fname}")
